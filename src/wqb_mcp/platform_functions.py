...
def load_config() -> Dict[str, Any]:
    """Load configuration from .env file or JSON config file.

    Priority:
    1. Environment variables from .env file (WQB_EMAIL, WQB_PASSWORD)
    2. JSON config file (specified by MCP_CONFIG_FILE or default location)

    Returns:
        Config dict with optional "_source" metadata field indicating where credentials came from
    """
    config = {}

    # Try to load from .env file in current working directory first
    ...
    if email and password:
        ...
        config["_source"] = "env"  # Mark that credentials came from .env
    ...
    if os.path.exists(config_file):
        try:
            with open(config_file, 'r', encoding='utf-8') as f:
                config = json.load(f)
                config["_source"] = "json"  # Mark that credentials came from JSON
...
async def authenticate(email: Optional[str] = "", password: Optional[str] = "") -> Dict[str, Any]:
    ...
    try:
        ...
        config_source = config.get("_source", "direct")
        ...
        # Only save credentials to JSON if they didn't come from .env
        # This prevents duplication and keeps .env as the single source of truth
        if auth_result.get('status') == 'authenticated' and config_source != "env":
            if 'credentials' not in config:
                config['credentials'] = {}
            config['credentials']['email'] = email
            config['credentials']['password'] = password
            # Remove _source metadata before saving
            config.pop("_source", None)
            save_config(config)
            logger.info("Credentials saved to JSON config")
        elif config_source == "env":
            logger.info("Credentials from .env - not saving to JSON config")
